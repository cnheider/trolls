...
def text_to_button(
    msg, color, button_x, button_y, button_width, button_height, size="vsmall"
):
    """
    """
...
def tank(x, y, tur_pos):
    ...
    pygame.draw.line(
        game_display, blue, (x, y), possible_turrets[tur_pos], turret_width
    )
...
def button(
    text, x, y, width, height, inactive_color, active_color, action=None, size=" "
):
    """
    """
...
def fire_shell(
    ...
    while fire:
        ...
        if starting_shell[1] > display_height - ground_height:
            ...
            hit_x = int(
                (starting_shell[0] * display_height - ground_height) / starting_shell[1]
            )
...
def e_fire_shell(
    ...
    while not power_found:
        ...
        while fire:
            ...
            if starting_shell[1] > display_height - ground_height:
                hit_x = int(
                    (starting_shell[0] * display_height - ground_height)
                    / starting_shell[1]
                )
                hit_y = int(display_height - ground_height)
                # explosion(hit_x,hit_y)
    ...
    while fire:
        ...
        gun_power = random.randrange(
            int(current_power * 0.90), int(current_power * 1.10)
        )
...
def game_loop():
    ...
    fire_power = 50
    power_change = 0
    x_location = (display_width / 2) + random.randint(
        -0.1 * display_width, 0.1 * display_width
    )
